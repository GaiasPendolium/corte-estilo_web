...
const imprimirFacturaHtml = ({ tipo, data, estilistas }) => {
  const logoSrc = `${window.location.origin}/corte_estilo_logo.png`;
  const fecha = formatPrintDate(data?.fecha_hora);
  const numeroFactura = data?.numero_factura || data?.id || '-';
  const cliente = data?.cliente_nombre || 'Cliente no registrado';
  const medioPago = data?.medio_pago || '-';
  const usuario = data?.usuario_nombre || '-';
  const empleado = data?.estilista_nombre || '-';

  let detalleHtml = '';
  let resumenHtml = '';

  if (tipo === 'venta') {
    const items = Array.isArray(data?.items) && data.items.length > 0 ? data.items : [data];
    const total = items.reduce((acc, it) => acc + Number(it?.total || 0), 0);

    detalleHtml = items.map((it) => {
      const cantidad = Number(it?.cantidad || 1);
      const unitario = Number(it?.precio_unitario || 0);
      const totalItem = Number(it?.total || (cantidad * unitario));
      return `
        <tr>
          <td>${escapeHtml(it?.producto_nombre || 'Producto')}</td>
          <td class="num">${cantidad}</td>
          <td class="num">${formatPrintMoney(unitario)}</td>
          <td class="num">${formatPrintMoney(totalItem)}</td>
        </tr>
      `;
    }).join('');
    ...
    resumenHtml = `
      <div class="summary-line total">
        ...
        <strong>${formatPrintMoney(total)}</strong>
      </div>
    `;
  } else {
    const estilistaCfg = (estilistas || []).find((e) => Number(e.id) === Number(data?.estilista));
    const tipoAuto = estilistaCfg?.tipo_cobro_espacio || 'sin_cobro';
    const valorAuto = Number(estilistaCfg?.valor_cobro_espacio || 0);
    const tipoReparto = data?.tipo_reparto_establecimiento || '';
    const valorReparto = Number(data?.valor_reparto_establecimiento || 0);
    const origenReparto = tipoReparto
      ? `${tipoReparto} ${tipoReparto === 'porcentaje' ? `${valorReparto}%` : formatPrintMoney(valorReparto)}`
      : `Automatico: ${tipoAuto}${tipoAuto === 'porcentaje_neto' ? ` ${valorAuto}%` : tipoAuto === 'costo_fijo_neto' ? ` ${formatPrintMoney(valorAuto)}` : ''}`;

    const valorServicio = Number(data?.neto_servicio ?? data?.precio_cobrado ?? 0);
    const valorEstablecimiento = Number(data?.monto_establecimiento || 0);
    const valorEmpleado = Number(data?.monto_estilista || 0);
    const valorAdicionales = Number(data?.valor_adicionales || 0);
    const totalCliente = Number(data?.precio_cobrado || 0) + valorAdicionales;

    detalleHtml = `
      <tr>
        <td>${escapeHtml(data?.servicio_nombre || 'Servicio')}</td>
        <td class="num">1</td>
        <td class="num">${formatPrintMoney(valorServicio)}</td>
        <td class="num">${formatPrintMoney(valorServicio)}</td>
      </tr>
      ${valorAdicionales > 0 ? `
      <tr>
        <td>Adicionales</td>
        <td class="num">1</td>
        <td class="num">${formatPrintMoney(valorAdicionales)}</td>
        <td class="num">${formatPrintMoney(valorAdicionales)}</td>
      </tr>
      ` : ''}
    `;
    ...
    resumenHtml = `
      <div class="summary-line"><span>Reparto establecimiento</span><strong>${escapeHtml(origenReparto)}</strong></div>
      <div class="summary-line"><span>Valor para empleado</span><strong>${formatPrintMoney(valorEmpleado)}</strong></div>
      <div class="summary-line"><span>Valor para establecimiento</span><strong>${formatPrintMoney(valorEstablecimiento)}</strong></div>
      <div class="summary-line total"><span>Total pagado por cliente</span><strong>${formatPrintMoney(totalCliente)}</strong></div>
    `;
  }
  ...
  const html = `
    ...
    <html lang="es">
      <head>
        <meta charset="utf-8" />
        <title>Factura ${escapeHtml(numeroFactura)}</title>
        <style>
          * { box-sizing: border-box; }
          body { font-family: Arial, sans-serif; margin: 0; padding: 24px; color: #111827; }
          .invoice { max-width: 780px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden; }
          .header { background: #f8fafc; padding: 18px 20px; border-bottom: 1px solid #e5e7eb; text-align: center; }
          .logo { max-height: 72px; max-width: 220px; object-fit: contain; display: block; margin: 0 auto 8px; }
          .title { margin: 0; font-size: 20px; font-weight: 700; }
          .meta { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px 16px; padding: 16px 20px; font-size: 13px; }
          .meta strong { color: #111827; }
          .table-wrap { padding: 0 20px 14px; }
          table { width: 100%; border-collapse: collapse; font-size: 13px; }
          th, td { border: 1px solid #e5e7eb; padding: 8px; text-align: left; }
          th { background: #f1f5f9; }
          .num { text-align: right; }
          .summary { border-top: 1px solid #e5e7eb; background: #f8fafc; padding: 14px 20px; }
          .summary-line { display: flex; justify-content: space-between; gap: 12px; margin-bottom: 6px; font-size: 14px; }
          .summary-line.total { font-size: 16px; font-weight: 700; margin-top: 10px; }
          .footer { border-top: 1px solid #e5e7eb; padding: 10px 20px 16px; color: #475569; font-size: 12px; }
          @media print {
            body { padding: 0; }
      ...
      <body>
        <section class="invoice">
          <header class="header">
            <img class="logo" src="${logoSrc}" alt="Logo Corte y Estilo" />
            <h1 class="title">${tipo === 'venta' ? 'Factura de Producto' : 'Factura de Servicio'}</h1>
          </header>
          <section class="meta">
            <div><strong>Factura:</strong> ${escapeHtml(numeroFactura)}</div>
            <div><strong>Fecha:</strong> ${escapeHtml(fecha)}</div>
            <div><strong>Cliente:</strong> ${escapeHtml(cliente)}</div>
            <div><strong>Empleado:</strong> ${escapeHtml(empleado)}</div>
            <div><strong>Usuario que facturó:</strong> ${escapeHtml(usuario)}</div>
            <div><strong>Medio de pago:</strong> ${escapeHtml(medioPago)}</div>
          </section>
          ...
          <section class="summary">${resumenHtml}</section>
